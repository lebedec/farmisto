...
impl Base {
    ...
    pub fn new(window: &Window, mut extensions: Vec<CString>) -> Self {
        unsafe {
            ...
            debug!("Creates debug utils messenger");
            ...
            debug!("Enumerates physical devices");
            let pdevices = instance
                .enumerate_physical_devices()
                .expect("Unable to enumerate physical devices");

            let physical_devices_search = |device: &PhysicalDevice| {
                let properties = instance.get_physical_device_properties(*device);
                let mut chars = vec![];
                for c in properties.device_name {
                    if c == '\0' as i8 {
                        break;
                    }
                    chars.push(c as u8);
                }
                let name = String::from_utf8(chars);
                debug!("Gets physical device {name:?} queue support");
                instance
                    .get_physical_device_queue_family_properties(*device)
                    .iter()
                    .enumerate()
                    .find_map(|(index, info)| {
                        let is_graphics = info.queue_flags.contains(vk::QueueFlags::GRAPHICS);
                        let surface_support = surface_loader
                            .get_physical_device_surface_support(*device, index as u32, surface)
                            .unwrap_or(false);
                        if is_graphics && surface_support {
                            Some((*device, index))
                        } else {
                            None
                        }
            ...
            debug!("Finds suitable physical device");
            let (physical_device, queue_family_index) = pdevices
                .iter()
                .find_map(physical_devices_search)
                .expect("Unable to find suitable physical device");
            ...
            debug!("Allocates command buffers");
            let command_buffers = device
                .allocate_command_buffers(&command_buffer_allocate_info)
                .expect("Unable to allocate command buffers");

            // synchronization
            debug!("Starts synchronization");
