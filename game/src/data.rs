...
impl Game {
    ...
    pub fn load_game_knowledge(&mut self) {
        ...
        for entry in self.storage.fetch_all::<SpaceKind>().into_iter() {
            ...
            self.physics.known_spaces.insert(space.id, Shared::new(space));
        }
        ...
        for entry in self.storage.fetch_all::<BarrierKind>().into_iter() {
            ...
            self.physics.known_barriers.insert(barrier.id, Shared::new(barrier));
        }
        ...
        for entry in self.storage.fetch_all::<PlantKind>().into_iter() {
            ...
            self.planting.known_plants.insert(plant.id, Shared::new(plant));
        }
        ...
        for entry in self.storage.fetch_all::<FarmlandKind>().into_iter() {
            ...
            self.universe.known.farmlands.insert(farmland.id, Shared::new(farmland));
        }
        for entry in self.storage.fetch_all::<FarmerKind>().into_iter() {
            let farmer = self.load_farmer_kind(entry).unwrap();
            self.universe.known.farmers.insert(farmer.id, Shared::new(farmer));
        }
    ...
    pub fn load_game_state(&mut self) {
        ...
        // physics
        self.physics.spaces = self.storage.fetch_all::<Space>().into_iter().map(|entry|
            self.load_space(entry).unwrap()).collect();
        ...
        // planting
        self.planting.lands = self.storage.fetch_all::<Land>().into_iter().map(|entry|
            self.load_land(entry).unwrap()).collect();
        ...
        // models
        self.universe.trees = self.storage.fetch_all::<Tree>().into_iter().map(|entry|
            self.load_tree(entry).unwrap()).collect();
        self.universe.farmlands = self.storage.fetch_all::<Farmland>().into_iter().map(|entry|
            self.load_farmland(entry).unwrap()).collect();
        self.universe.farmers = self.storage.fetch_all::<Farmer>().into_iter().map(|entry|
            self.load_farmer(entry).unwrap()).collect();
    ...
    pub(crate) fn load_farmland_kind(&mut self, entry: Entry) -> Result<FarmlandKind, serde_json::Error> {
        let id = entry.get("id")?;
    ...
    pub(crate) fn load_farmland(&mut self, entry: Entry) -> Result<Farmland, serde_json::Error> {
        ...
        let data = Farmland {
            id: FarmlandId(id),
            kind: self.universe.known.farmlands.get(&FarmlandKey(kind)).unwrap().clone(),
        };
        Ok(data)
    }

    pub(crate) fn load_farmer_kind(&mut self, entry: Entry) -> Result<FarmerKind, serde_json::Error> {
        let id = entry.get("id")?;
    ...
    pub(crate) fn load_farmer(&mut self, entry: Entry) -> Result<Farmer, serde_json::Error> {
        ...
        let data = Farmer {
            id: FarmerId(id),
            kind: self.universe.known.farmers.get(&FarmerKey(kind)).unwrap().clone(),
    ...
    pub(crate) fn load_tree(&mut self, entry: Entry) -> Result<Tree, serde_json::Error> {
        ...
        let data = Tree {
            id: TreeId(id),
            kind: self.universe.known.trees.get(&TreeKey(kind)).unwrap().clone(),
        };
        Ok(data)
    }
    ...
    pub(crate) fn load_space(&mut self, entry: Entry) -> Result<Space, serde_json::Error> {
        ...
        let data = Space {
            id: SpaceId(id),
            kind: self.physics.known_spaces.get(&SpaceKey(kind)).unwrap().clone(),
        };
        Ok(data)
    }
    ...
    pub(crate) fn load_body(&mut self, entry: Entry) -> Result<Body, serde_json::Error> {
        ...
        let data = Body {
            id: BodyId(id),
            kind: self.physics.known_bodies.get(&BodyKey(kind)).unwrap().clone(),
    ...
    pub(crate) fn load_barrier_kind(&mut self, entry: Entry) -> Result<BarrierKind, serde_json::Error> {
        let id = entry.get("id")?;
    ...
    pub(crate) fn load_barrier(&mut self, entry: Entry) -> Result<Barrier, serde_json::Error> {
        ...
        let data = Barrier {
            id: BarrierId(id),
            kind: self.physics.known_barriers.get(&BarrierKey(kind)).unwrap().clone(),
            position: entry.get("position")?,
            space: SpaceId(space),
        };
        Ok(data)
    ...
    pub(crate) fn load_land(&mut self, entry: Entry) -> Result<Land, serde_json::Error> {
        let id = entry.get("id")?;
        let kind = entry.get("kind")?;
        let data = Land {
            id: LandId(id),
            kind: self.planting.known_lands.get(&LandKey(kind)).unwrap().clone(),
        };
        Ok(data)
    }
    ...
    pub(crate) fn load_plant(&mut self, entry: Entry) -> Result<Plant, serde_json::Error> {
        ...
        let data = Plant {
            id: PlantId(id),
            kind: self.planting.known_plants.get(&PlantKey(kind)).unwrap().clone(),
            land: LandId(land),
        };
        Ok(data)
    }
